...
void insertIntoTablesOld(int index, char c, int value,std::vector<std::vector<ActionValue>>& ActionTable, std::vector<std::vector<int>> GotoTable) {
    if (isNT(c)) {
...
int main() {
    ...
    std::vector<std::vector<ActionValue>> ActionTable(10, std::vector<ActionValue>(5));
    std::vector<std::vector<int>> GotoTable(10, std::vector<int>(5));
            ...
            if (iOfDot != g.rhs.length() - 1) {
                ...
                for (int i = 0; i<closures.size(); ++i) {
                    if (compareClosures(newClosure, closures[i])) {
                        flag = true;
                        //insertIntoTablesOld(index, g.rhs.at(iOfDot+1), i, ActionTable, GotoTable);
                        break;
                    }
                }
                if (!flag) {
                    ...
                    //insertIntoTablesOld(index, g.rhs.at(iOfDot+1), max, ActionTable, GotoTable);
                }
